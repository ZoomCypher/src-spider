...
class PurseMysql(object):
    ...
    def request_top_domain(self, top_domain):
        ...
        response = requests.get(top_domain_url, headers=self.headers, timeout=0.5)
    ...
    def get_second_domain(self, domain):
        ...
        # 处理特殊网站
        if '新华网' in title[:3]:
            title = re.split(self.split_pattern, title)[-1]
        elif '新华网' in title[3:]:
            title = re.split(self.split_pattern, title)[0]
        else:
            title = re.split(self.split_pattern, title)[0]
    ...
    def update_title_to_mysql(self, element):
        ...
        try:
            self.cursor = self.db.cursor()
            judge_sql = 'select name from t_site_base where name = "%s"' % (title)
            self.cursor.execute(judge_sql)
            title_name = self.cursor.fetchone()
            if title_name == None: # 数据库没有信息则不处理
                pass
            else: # 数据库有信息则加上域名（名称-域名）
                title = title + '-' + domain
        except:
            pass
        ...
        if '-.-' in title:
            try:
                ...
                print '%s update:%s' % (time_stamp.strftime('%Y.%m.%d %H:%M:%S'),  remark)
                # 确认事务
                self.db.commit()
            ...
            try:
                ...
                self.cursor.execute(sql_update)
                time_stamp = datetime.datetime.now()
                print '%s update:%s' % (time_stamp.strftime('%Y.%m.%d %H:%M:%S'), element)
                # 确认事务
                self.db.commit()
            except Exception as e:
                print e
    ...
    def syn_other_table(self, title, nowTime, domain):
        ...
        try:
            ...
            self.db.commit()
            time_stamp = datetime.datetime.now()
            print '%s syn date %s to t_site_syn' % (time_stamp.strftime('%Y.%m.%d %H:%M:%S'), title)
        except Exception as e:
            pass
    ...
    def get_top_domain(self, domain):
        ...
        web_name = re.split(self.split_pattern, web_name)[0]
        
        if len(web_name) >= 10:
            web_name = web_name[:6]
        
        if re.search(u'[^\u4e00-\u9fa5]', web_name):
            web_name = '-'
        
        if 'IP地址查询' in web_name:
            web_name = '-'
    ...
    def sync_domian_with_mysql(self):
        ...
        try:
            ...
            if element == None:
                time_stamp = datetime.datetime.now()
                print '%s refreshing mysql and waiting for new data......' % time_stamp.strftime('%Y.%m.%d %H:%M:%S') 
                self.refresh()
                time.sleep(10)
                return
    ...
    def refresh(self):
        try:
            ...
            print 'refresh failed, waiting %s' % e
            time.sleep(60)
            pass
